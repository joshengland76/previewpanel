// Phase B4b, Task 4 -- 120-video confirmation run (arms N1 + S only; the
// pilot already established the same-prompt noise floor via N1/N2, no need
// to re-run it here). Same hardened harness as the pilot:
...
//     readiness guard the live app uses), then REUSED across both arms x 3
//     judges = 6 analyze calls for that video, instead of re-uploading per
...
const SAMPLE_PATH = process.env.CONFIRM_SAMPLE || path.join(SNAPSHOT_DIR, "phaseb4b_confirm_sample.json");
const OUT_PATH = process.env.CONFIRM_OUT || path.join(SNAPSHOT_DIR, "phaseb4b_confirm_results.json");
